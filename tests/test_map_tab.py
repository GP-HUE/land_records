...
def main():
    ...
    checks = [
        ...
        ("mapSetTileSource function (with auto flag)", "function mapSetTileSource(key, auto)"),
        ("auto tile-fallback source order", "const TILE_SOURCE_ORDER = ['osm', 'osm_de', 'esri', 'carto', 'topo']"),
        ("tile source: OSM mirror (openstreetmap.de)", "tile.openstreetmap.de"),
        ("tile source: OSM mirror subdomains", "subdomains: 'abc'"),
        ("tile source: OSM mirror in dropdown", 'value="osm_de"'),
        ("auto tile-fallback on tileerror", "trying " ),
        ("all-sources-failed -> schematic offer", "No map source could load tiles"),
        ...
        # show-mode toggle (selected-only default / all records)
        ...
        ("backup restore flow", "restoreBackup()"),
        ("backup API: export", "/api/backup/export"),
        ("backup API: import", "/api/backup/import"),
        # manual-coordinate digitize (accuracy)
        ("digitize panel element", 'id="mapDigitizePanel"'),
        ("digitize: manual lat/lon add", "dz-new-lat"),
        ("digitize: per-corner editable inputs", "dz-lat"),
        ("digitize: manual add function", "function mapDigitizeAddManual"),
        ("digitize: edit existing corner", "function mapDigitizeEditPoint"),
        ("digitize: re-digitize pre-fills existing corners", "Re-digitize accuracy"),
    ]
